makesummaryq<-function(corpus, qwerty, dec_tree)
{
print("makesummary")
  ptm<-proc.time()
...
##print(length(tf_idf1))
  df1<-data.frame(sentences=sentences1,parano=parano1,paraoffset=paraoffset1,propernoun=propernoun1,termfrequency=termfrequency1,lexical = lexical1, tf_idf = tf_idf1)
  #return(df[,2])
  summary <- c()
  for(i in 1:length(spans_of_sentences))
